...
# Alarm발생시간 및 경과시간
CONST_ALARM_ELASPED_TIME = 30 #30분 후에 다시 SMS발송한다.
...
pubMessage.pub_MQTT("화재감지기 복구설정 완료","cloud","ok")
...
pubMessage.pub_MQTT("정전 복구설정 완료","cloud","ok")

print("Switch Setting End")
pubMessage.pub_MQTT("1번존 설정완료","2번존 설정완료","전체경계 설정완료")
...
#============================================================= zone1 close       
        if GPIO.input(GPIO_IN_SECURITY) == 0 and flag_Security == 0:            
            ...
            #mqtt 문자발송
            pubMessage = PUB.MQTTPublisher()
            pubMessage.pub_MQTT("화재감지기가 복구되었습니다.!!","cloud","ok")
